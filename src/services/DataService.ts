...
export class DataService {
    @observable
    registrationStatus: AsyncData<RegistrationApiResponse | undefined> = createAsyncData()

    @observable
    events: AsyncData<EventModel[]> = createAsyncData()

    // Caches registration data per session storage.
    fetchRegistrationStatus(currentUser: firebase.User, force: boolean = false): Promise<void> {
